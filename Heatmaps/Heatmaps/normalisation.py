"""Three-channel input normalisation utilities for heatmap models."""
...
class ChannelStatistics:
    ...
    def update(self, image):
        """Add one channel-first image to the estimate."""
        values = np.asarray(image, dtype=np.float64)

        if values.ndim != 3 or values.shape[0] != self.channels:
            raise ValueError(
                f'Input normalisation requires channel-first images with exactly {self.channels} channels, got shape {values.shape}.'
            )

        values = values.reshape(self.channels, -1)
...
def normalise_channel_first(image, mean, standard_deviation):
    """Apply per-channel normalisation to one channel-first image."""
    mean, standard_deviation = validate_normalisation_constants(mean, standard_deviation)
    image = np.asarray(image, dtype=np.float32)

    if image.ndim != 3 or image.shape[0] != EXPECTED_NORMALISATION_CHANNELS:
        raise ValueError(
            f'Input normalisation requires channel-first images with exactly {EXPECTED_NORMALISATION_CHANNELS} channels, '
            f'got shape {image.shape}.'
        )

    mean_array = np.asarray(mean, dtype=np.float32)[:, np.newaxis, np.newaxis]
    std_array = np.asarray(standard_deviation, dtype=np.float32)[:, np.newaxis, np.newaxis]
    return ((image - mean_array) / std_array).astype(np.float32)
